...
`OptionalInstanceSource` makes the wrapper's `image` and `binary` overrides
mutually exclusive while allowing neither, which uses its built-in `mychaind`
binary. Use `InstanceSource` instead when a custom wrapper has no default and
must require exactly one source. `cosmosBase` itself still receives both an
optional image and the executable name to invoke inside that image.
