...
export type DockerOptions = {
  ...
  homeDir: string
  /** Files and environment shared by every chain CLI invocation. */
...
function mountArgs({ homeDir, runtime }: DockerOptions): string[] {
  ...
  return [
    ...
    ...(runtime?.mounts ?? []).flatMap(({ source, target, readOnly }) => [
